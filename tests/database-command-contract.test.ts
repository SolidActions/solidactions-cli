...
const COMMANDS = [
    ...
    { verb: 'export', args: [['name', true]], options: ['--table', '--output', '--no-wait', '--resume', '--replace', '--json'] },
...
const CRITICAL_OPTIONS = ['--json', '--from', '--yes', '--writable', '--resume', '--kind', '--no-wait', '--limit', '--table', '--mode', '--batch-id', '--wait', '--output', '--replace'];
